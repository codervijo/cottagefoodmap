...
| **v2.B** | states batch 1 | Pennsylvania, Illinois, Georgia, North Carolina, Michigan at the v1 data standard; +65 pages (13 per state); growth.md entry with baseline | planned |
...
- **v2** — Which 15 states to add, and by what criterion (GSC impressions, keyword demand, population, law clarity)? — *answered 2026-09-25:* the 15 most populous uncovered states (Census 2024 estimates), in population order across v2.B–v2.D. Keyword demand was the preferred criterion but couldn't be measured (Ahrefs units exhausted; GSC has no queries for uncovered states). Batch order can be revisited with keyword data before v2.C.
- **v2** — Per-state sourcing workflow: how a new state gets researched, cited, and reviewed. — *answered 2026-09-25:* same as v1.D: one research agent per state works from official sources only (agency page, statute, official PDF); key quotes are re-fetched and checked before editing; facts that can't be confirmed are `Unverified`; the operator reviews the diff before commit.
- **v2** — Does the 7-category food list or the `StateLaw` schema need to change to fit new states? — *answered 2026-09-25:* no change planned up front. If a state doesn't fit, the batch that hits it decides the change and records an ADR.
