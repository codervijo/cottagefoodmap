...
export const Route = createFileRoute("/states/")({
  head: () => ({
    meta: [
      { title: "Cottage Food Laws by State — Cottage Food Law /US" },
      { name: "description", content: "Browse and filter cottage food laws by state: permit requirements, sales caps, online sales, and allowed foods." },
      { property: "og:title", content: "Cottage Food Laws by State" },
      { property: "og:description", content: "Browse and filter cottage food laws by state." },
...
// Tailwind utility — extracted via @apply isn't needed; inline class is fine.
const _i =
  "h-9 rounded border border-input bg-background px-2 text-sm text-foreground focus:border-ring focus:outline-none focus:ring-2 focus:ring-ring/30";
// Apply via a small CSS injection trick: re-export so Tailwind keeps classes if referenced.
void _i;
