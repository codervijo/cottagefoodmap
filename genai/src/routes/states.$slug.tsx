...
import { FactRow, YesNo, MoneyOrNone, BulletList } from "@/components/Fact";
...
export const Route = createFileRoute("/states/$slug")({
  ...
  head: ({ params, loaderData }) => {
    const name = loaderData?.state.name ?? params.slug;
    return {
      meta: [
        { title: `${name} Cottage Food Law — Permits, Sales Cap, Allowed Foods` },
        { name: "description", content: `Cottage food law in ${name}: permit requirements, license cost, sales cap, allowed and prohibited foods, labeling, and sales channels. Sourced.` },
        { property: "og:title", content: `${name} Cottage Food Law` },
        { property: "og:description", content: `Sourced reference to ${name} cottage food rules.` },
  ...
  notFoundComponent: () => (
    <div className="container-prose py-16">
      ...
      <p className="mt-2 text-muted-foreground">
        We don't have a page for that state yet.{" "}
        <Link to="/states">Browse covered states</Link>.
      </p>
    </div>
  ),
});
    ...
    <article className="container-prose py-10">
      ...
      <div className="double-rule mt-10 pt-6">
        ...
        <ul className="mt-3 grid grid-cols-1 gap-2 text-sm md:grid-cols-2">
          <li>
            <Link to="/guides/license-cost/$state" params={{ state: state.slug }}>
              Cottage food license cost in {state.name}
            </Link>
          </li>
          <li>
            <Link to="/guides/labeling/$state" params={{ state: state.slug }}>
              Cottage food labeling requirements in {state.name}
            </Link>
          </li>
          {otherStates.map((s: any) => (
            <li key={s.slug}>
              <Link to="/states/$slug" params={{ slug: s.slug }}>Compare with {s.name}</Link>
            </li>
          ))}
        </ul>
      </div>
