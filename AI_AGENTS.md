# AI Agent Context — cottagefoodmap.com
...
`proj=cottagefoodmap.com`, so these all work either from this dir or from `sites/`:

| Command | What it does |
|---|---|
| `make buildsh` *(from `sites/`)* | Drop into a bash shell inside the docker container at `/usr/src/app` (= `sites/` mounted in). |
| `make run` *(from here)* / `make run proj=cottagefoodmap.com` *(from `sites/`)* | `pnpm install` then start dev server (auto-detected). |
| `make check-vite proj=cottagefoodmap.com` | Start the dev server, skipping install. |
| `make test proj=cottagefoodmap.com` | `pnpm install` + `pnpm build` + `pnpm test`. **Hard-fails outside docker** — `make buildsh` first, or `docker exec`. |
...
docker exec -w /usr/src/app <name> make test proj=cottagefoodmap.com
```
...
- **Deploy:** `wrangler deploy` (locally) or via Cloudflare's Git integration on push.
  ...
  `cd ../portfolio && make run ARGS="deploy cottagefoodmap.com"` runs `gh repo create` and
...
`portfolio project check cottagefoodmap.com` (run from `sites/portfolio/`).
